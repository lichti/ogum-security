...
describe('ProvidersTable', () => {
  ...
  it('renders empty state when no providers', () => {
    render(<ProvidersTable providers={[]} onEdit={noop} onToggle={noop} onDiscover={noop} onScan={noop} onDelete={noop} />)
    expect(screen.getByText(/no cloud accounts connected/i)).toBeInTheDocument()
  })

  it('renders provider row with all columns', () => {
    render(<ProvidersTable providers={[makeProvider()]} onEdit={noop} onToggle={noop} onDiscover={noop} onScan={noop} onDelete={noop} />)
  ...
  it('shows Active status badge for active provider', () => {
    render(<ProvidersTable providers={[makeProvider({ status: 'active' })]} onEdit={noop} onToggle={noop} onDiscover={noop} onScan={noop} onDelete={noop} />)
    expect(screen.getByText('Active')).toBeInTheDocument()
  })

  it('shows Disabled status badge for disabled provider', () => {
    render(<ProvidersTable providers={[makeProvider({ status: 'disabled', enabled: false })]} onEdit={noop} onToggle={noop} onDiscover={noop} onScan={noop} onDelete={noop} />)
    expect(screen.getByText('Disabled')).toBeInTheDocument()
  })

  it('shows Pending status badge for pending provider', () => {
    render(<ProvidersTable providers={[makeProvider({ status: 'pending' })]} onEdit={noop} onToggle={noop} onDiscover={noop} onScan={noop} onDelete={noop} />)
  ...
  it('truncates regions and shows overflow count', () => {
    ...
    render(<ProvidersTable providers={[provider]} onEdit={noop} onToggle={noop} onDiscover={noop} onScan={noop} onDelete={noop} />)
    expect(screen.getByText(/\+1/)).toBeInTheDocument()
  })

  it('calls onDiscover when refresh button is clicked', async () => {
    const onDiscover = jest.fn().mockResolvedValue(undefined)
    render(<ProvidersTable providers={[makeProvider()]} onEdit={noop} onToggle={noop} onDiscover={onDiscover} onScan={noop} onDelete={noop} />)
  ...
  it('calls onDelete when delete button is clicked', async () => {
    ...
    render(<ProvidersTable providers={[makeProvider()]} onEdit={noop} onToggle={noop} onDiscover={noop} onScan={noop} onDelete={onDelete} />)
  ...
  it('calls onToggle with false when disable button clicked on enabled provider', async () => {
    ...
    render(<ProvidersTable providers={[makeProvider({ enabled: true })]} onEdit={noop} onToggle={onToggle} onDiscover={noop} onScan={noop} onDelete={noop} />)
  ...
  it('calls onToggle with true when enable button clicked on disabled provider', async () => {
    const onToggle = jest.fn().mockResolvedValue(undefined)
    render(<ProvidersTable providers={[makeProvider({ enabled: false, status: 'disabled' })]} onEdit={noop} onToggle={onToggle} onDiscover={noop} onScan={noop} onDelete={noop} />)
  ...
  it('renders multiple providers', () => {
    ...
    render(<ProvidersTable providers={providers} onEdit={noop} onToggle={noop} onDiscover={noop} onScan={noop} onDelete={noop} />)
  ...
  it('calls onScan when scan button is clicked', async () => {
    ...
    render(<ProvidersTable providers={[makeProvider()]} onEdit={noop} onToggle={noop} onDiscover={noop} onScan={onScan} onDelete={noop} />)
    fireEvent.click(screen.getByTitle('Run CSPM scan now'))
    await waitFor(() => expect(onScan).toHaveBeenCalledWith('aws-111111111111'))
  })

  it('scan button is disabled when provider is disabled', () => {
    render(
      <ProvidersTable
        providers={[makeProvider({ enabled: false, status: 'disabled' })]}
        onEdit={noop}
    ...
    expect(screen.getByTitle('Run CSPM scan now')).toBeDisabled()
  })
})
