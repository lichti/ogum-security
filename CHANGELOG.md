...
- **AWSGoat vendored as an end-to-end vulnerable-app test target (`infra/terraform/awsgoat`)**: `module-1` (serverless blog: Lambda + API Gateway + DynamoDB + S3, XSS/SQLi/IDOR/SSRF chained into IAM privilege escalation via IMDS credential theft) and `module-2` (ECS/Fargate HR payroll app, SQLi/file-upload chained into container breakout + IAM privilege escalation), vendored unmodified from [ine-labs/AWSGoat](https://github.com/ine-labs/AWSGoat) (MIT, commit `b24869a`) along with its attack manuals and minimum deploy IAM policy. Complements `test-fixtures/` (synthetic, no running app) by exercising the full chain Ogum needs to detect end-to-end — Ogum.Dynamic catching the app-layer vulnerability, Ogum.Pulse/CDR the resulting cloud API activity, Ogum.Graph the reconstructed attack path — rather than only CSPM misconfigurations. Deliberately excludes upstream's GitHub Actions auto-deploy workflows (manual `terraform apply` only, consistent with this repo's no-CI-cloud-deploy posture); each module deploys as its own root module with no `suffix` variable, intended for a dedicated disposable sandbox account, one module at a time. See `infra/terraform/awsgoat/README.md` for safety notes — these are real, deliberately internet-reachable exploitable applications, not simulated findings.
